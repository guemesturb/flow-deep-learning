...
def main():
    ...
    X = np.zeros((X_hr_target.shape[0] * Nx * Nz, Px * Pz * X_hr_target.shape[1]))

    n_t = X_hr_target.shape[0]

    nS = 0

    for t in range(X_hr_target.shape[0]):
        ...
        for i in range(Nx):
            ...
            for k in range(Nz):
                ...
                X[nS, :] = np.concatenate(
                    (
                        X_hr_target[t, 0, Pz*k:Pz*(k+1), Px*i:Px*(i+1)].flatten(), 
                        X_hr_target[t, 1, Pz*k:Pz*(k+1), Px*i:Px*(i+1)].flatten(), 
                        X_hr_target[t, 2, Pz*k:Pz*(k+1), Px*i:Px*(i+1)].flatten()
                    )
                )
    ...
    print(X_hr_target.shape)

    with np.load(f'wallEPODbase_yp{yp_flow:03d}.npz') as data:

        L     = data['L']
        basis = data['basis']

    psi_test = np.matmul(X_hr_target, basis)

    Y_pred = np.matmul(psi_test, L)
    ...
    Y_pred_u = np.zeros((3125, Nz*Pz, Nx*Px))
    Y_pred_v = np.zeros((3125, Nz*Pz, Nx*Px))
    Y_pred_w = np.zeros((3125, Nz*Pz, Nx*Px))
    ...
    for t in range(n_samp):
        ...
        for i in range(Nx):
            ...
            for k in range(Nz):
                ...
                Y_pred_u[t, Pz*k:Pz*(k+1), Px*i:Px*(i+1)] = np.reshape(Y_pred[nS, :256], (Pz, Px)) / std_out[0]
                Y_pred_v[t, Pz*k:Pz*(k+1), Px*i:Px*(i+1)] = np.reshape(Y_pred[nS, 256:512], (Pz, Px)) / std_out[1]
                Y_pred_w[t, Pz*k:Pz*(k+1), Px*i:Px*(i+1)] = np.reshape(Y_pred[nS, 512:], (Pz, Px)) / std_out[2]
    ...
    axs[0,0].contourf(x_hr, z_hr, Y_hr_predic[100, 0, :, :], cmap='RdBu_r', levels=21, vmin=-2, vmax=2)
    axs[0,1].contourf(x_hr, z_hr, Y_hr_target[100, 0, :, :], cmap='RdBu_r', levels=21, vmin=-2, vmax=2)

    # axs[0,0].set_xlim([0, Lx])
    # axs[0,0].set_ylim([0, Lz])
    # axs[0,1].set_xlim([0, Lx])
    # axs[0,1].set_ylim([0, Lz])
    ...
    fig, axs = plt.subplots(1, 3, figsize=(fig_width, fig_height), squeeze=False)

    nx = Y_hr_target.shape[3]
    nz = Y_hr_target.shape[2]
    kx = np.arange(1, nx / 2 + 0.1, 1)
    kz = np.arange(1, nz / 2 + 0.1, 1)
    kxx, kzz = np.meshgrid(kx,kz)
    lxx = Lx/kxx*Ret
    lzz = Lz/kzz*Ret

    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_target[:,0,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    E_true = np.max(phi)
    axs[0,0].contourf(lxx, lzz, phi/E_true, cmap='Greys', levels=[0.1,0.5,0.9], vmin=0, vmax=1)
    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_predic[:,0,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    axs[0,0].contour(lxx, lzz, phi/E_true, colors='r', levels=[0.1,0.5,0.9])

    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_target[:,1,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    E_true = np.max(phi)
    axs[0,1].contourf(lxx, lzz, phi/E_true, cmap='Greys', levels=[0.1,0.5,0.9], vmin=0, vmax=1)
    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_predic[:,1,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    axs[0,1].contour(lxx, lzz, phi/E_true, colors='r', levels=[0.1,0.5,0.9])

    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_target[:,2,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    E_true = np.max(phi)
    axs[0,2].contourf(lxx, lzz, phi/E_true, cmap='Greys', levels=[0.1,0.5,0.9], vmin=0, vmax=1)
    phi = kxx*kzz*np.mean(np.abs(np.fft.fft2(Y_hr_predic[:,2,:,:], axes=(1, 2)))**2, axis=0)[:int(nz / 2), :int(nx / 2)]
    axs[0,2].contour(lxx, lzz, phi/E_true, colors='r', levels=[0.1,0.5,0.9])
    ...
    axs[0,0].set_ylabel('$\\lambda_z^+$')
    axs[0,0].set_xlabel('$\\lambda_x^+$')
    axs[0,1].set_xlabel('$\\lambda_x^+$')
    axs[0,2].set_xlabel('$\\lambda_x^+$')
    fig.tight_layout()
    fig.savefig(f'test_spectra.pdf', dpi=600)
...
def tf_parser(rec):
    ...
    if subsampling_lr == 1:
        ...
        for i_comp in range(1, channels):
            ...
            inputs = tf.concat((inputs, tf.reshape((parsed_rec[f'wall_hr_raw{i_comp+1}']-avgs_in[i_comp]),(1,nz_hr, nx_hr))),0)
