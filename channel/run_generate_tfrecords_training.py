...
def main():
    ...
    Y_PD = process_POD(Y_HR, Nx, Nz, nx, nz, yp_flow, Ret)
    X_LR = preprocess_planes(yp_wall, n_samples, interv, nx, nz, h_size, dy_w, subsampling_lr, target=False)
    X_HR = preprocess_planes(yp_wall, n_samples, interv, nx, nz, h_size, dy_w, subsampling_hr, target=False)

    write_TFRecord(X_LR, X_HR, Y_HR, Y_PD, Ret, nx, nz, ny, interv, yp_flow, subsampling_lr, save_path, n_samples, max_samples_per_tfr)
...
def process_POD(Y, Nx, Nz, nx, nz, yp_flow, Ret):
    ...
    filename = f'/storage2/alejandro/urban/re{Ret}/PODbase_yp{yp_flow:03d}.npz'
    np.savez(filename, sigma=sigma, phi=phi)
    del X
    del phi

    Y = np.zeros((Y.shape[0], psi.shape[1], Nx, Nz))
