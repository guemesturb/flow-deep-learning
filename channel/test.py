...
def main():
    ...
    i_samp = np.zeros((n_samp, 1))

    itr = iter(dataset_test)
    import matplotlib.pyplot as plt
    for i in range(0, n_samp, batch_size):

        (X_lr_target[i:(i+batch_size), :, :, :], Y_hr_target[i:(i+batch_size), :, :, :], i_samp[i:(i+batch_size),0]) = next(itr)
        Y_hr_predic[i:(i+batch_size), :, :, :] = predictor.predict(X_lr_target[i:(i+batch_size), :, :, :])
    [print(i_samp[i,0]) for i in range(3125)]
    list1 = [int(i_samp[i,0]) for i in range(3125)]
    print(list1)
    Y_hr_target = Y_hr_target[isamp,:, :, :]
    plt.imshow(Y_hr_target[0,0, :, :])
    plt.savefig('test1.png')
    plt.imshow(Y_hr_target[1,0, :, :])
    plt.savefig('test2.png')
    plt.imshow(Y_hr_target[2,0, :, :])
...
def load_tfrecords(tfr_path, yp_flow, batch_size, n_prefetch, shuffle_buffer):
    ...
    [print(x) for x in tfr_files_test]
...
def tf_parser(rec):
    ...
    i_samp = tf.cast(parsed_rec['i_samp'], tf.int32)
