...
def main():
    ...
    model_name = f"{models_path}flow-reconstruction_fcnpod_mode_{mode+1:03d}.tf"
    ...
    psi_predic = np.zeros((n_samp))
    psi_target = np.zeros((n_samp))
    ...
    with np.load(avg_input_path) as data:
        ...
        avg_psi = data['avg_psi']
        std_psi = data['std_psi']
    ...
    for i in range(0, n_samp, batch_size):
        ...
        print(psi_target[i:(i+batch_size)])
        print(psi_predic[i:(i+batch_size)] * std_psi[mode] + avg_psi[mode])
        print(std_psi[mode])
        print(avg_psi[mode])
        jjjj
        # print('prediction')
    psi_predic = psi_predic * std_psi[mode] + avg_psi[mode]
    psi_predic = psi_predic - np.mean(psi_predic)
    psi_target = psi_target - np.mean(psi_target)
    # print(std_psi[mode])
    # print(np.std(psi_predic, axis=0))
    # print(psi_target.shape)
    # print(psi_predic.shape)
    R = np.mean(psi_target*psi_predic, axis=0) ** 2 / (np.std(psi_predic, axis=0) ** 2) / (np.std(psi_target, axis=0) ** 2)
    print(R)
    # jjjj
    # jjjj
    ...
    # print(Y_hr_target[0,0,17,10,:5])
    # print(Y_hr_predic[0,0,17,10,:5])
    ...
    # axs[0,0].imshow(Y_hr_target[0,0,17,:,:], cmap='RdBu_r', vmin=-1, vmax=1, origin='lower', extent=(0.25,1.25,0,2))
    # axs[0,1].imshow(Y_hr_predic[0,0,17,:,:], cmap='RdBu_r', vmin=-1, vmax=1, origin='lower', extent=(0.25,1.25,0,2))
    ...
    # fig, axs = plt.subplots(rows, cols, figsize=(fig_width, fig_height), squeeze=False)
    # axs[0,0].imshow(Y_hr_target[120,1,17,:,:], cmap='RdBu_r', vmin=-3, vmax=3, origin='lower', extent=(0.25,1.25,0,2))
    # axs[0,1].imshow(Y_hr_predic[120,1,17,:,:], cmap='RdBu_r', vmin=-3, vmax=3, origin='lower', extent=(0.25,1.25,0,2))
    # axs[0,0].set_xlabel('x/h')
    # axs[0,1].set_xlabel('x/h')
    # axs[0,0].set_ylabel('y/h')
    # axs[0,0].set_title('Target')
    # axs[0,1].set_title('Prediction')
    # fig.savefig(f'test1.pdf', dpi=600)

    # fig, axs = plt.subplots(rows, cols, figsize=(fig_width, fig_height), squeeze=False)
    # axs[0,0].imshow(Y_hr_target[210,2,17,:,:], cmap='RdBu_r', vmin=-3, vmax=3, origin='lower', extent=(0.25,1.25,0,2))
    # axs[0,1].imshow(Y_hr_predic[210,2,17,:,:], cmap='RdBu_r', vmin=-3, vmax=3, origin='lower', extent=(0.25,1.25,0,2))
    # axs[0,0].set_xlabel('x/h')
    # axs[0,1].set_xlabel('x/h')
    # axs[0,0].set_ylabel('y/h')
    # axs[0,0].set_title('Target')
    # axs[0,1].set_title('Prediction')
    # fig.savefig(f'test2.pdf', dpi=600)
...
def tf_parser(rec):
    ...
    psi = parsed_rec['flow_psi_raw'][mode]
