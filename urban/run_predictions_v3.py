...
def main():
    ...
    for mode in range(n_required_modes):
        ...
        predictors.append(keras.models.load_model(model_name))
    ...
    for i in range(0, n_samp, batch_size):
        ...
        for mode in range(n_required_modes):
            psi_predic[i:(i+batch_size),mode] = predictors[mode].predict(X_hr_target[i:(i+batch_size), :, :, :])
        # print(psi_target[i:(i+batch_size),:])
        # print(psi_predic[i:(i+batch_size),:] * std_psi[n_required_modes] + avg_psi[n_required_modes])
        # break
        # print('prediction')
    psi_predic = psi_predic * std_psi[:n_required_modes] + avg_psi[:n_required_modes]
